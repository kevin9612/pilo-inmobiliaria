 ...
 *
 * @method     ChildUsuariosQuery leftJoinPersonas($relationAlias = null) Adds a LEFT JOIN clause to the query using the Personas relation
 * @method     ChildUsuariosQuery rightJoinPersonas($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Personas relation
 * @method     ChildUsuariosQuery innerJoinPersonas($relationAlias = null) Adds a INNER JOIN clause to the query using the Personas relation
 *
 * @method     ChildUsuariosQuery joinWithPersonas($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Personas relation
 *
 * @method     ChildUsuariosQuery leftJoinWithPersonas() Adds a LEFT JOIN clause and with to the query using the Personas relation
 * @method     ChildUsuariosQuery rightJoinWithPersonas() Adds a RIGHT JOIN clause and with to the query using the Personas relation
 * @method     ChildUsuariosQuery innerJoinWithPersonas() Adds a INNER JOIN clause and with to the query using the Personas relation
 *
 ...
 * @method     ChildUsuarios findOneByTipo(int $tipo) Return the first ChildUsuarios filtered by the tipo column *
 ...
 * @method     ChildUsuarios requireOneByTipo(int $tipo) Return the first ChildUsuarios filtered by the tipo column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUsuarios[]|ObjectCollection findByTipo(int $tipo) Return ChildUsuarios objects filtered by the tipo column
...
abstract class UsuariosQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, login, password, tipo FROM usuarios WHERE id = :p0';
        try {
    ...
    public function filterByTipo($tipo = null, $comparison = null)
    {
        if (is_array($tipo)) {
            $useMinMax = false;
            if (isset($tipo['min'])) {
                $this->addUsingAlias(UsuariosTableMap::COL_TIPO, $tipo['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($tipo['max'])) {
                $this->addUsingAlias(UsuariosTableMap::COL_TIPO, $tipo['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByPersonas($personas, $comparison = null)
    {
        if ($personas instanceof \Personas) {
            return $this
                ->addUsingAlias(UsuariosTableMap::COL_ID, $personas->getIdUsuario(), $comparison);
